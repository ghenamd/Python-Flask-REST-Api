...
class Item(Resource):
    ...
    def get(self, name):
        ...
        if item:
            return item.json()
        return {'item': 'Item does not exist'}, 404  # Not found

    def post(self, name):
        if ItemModel.find_by_name(name):
            return {'message': "An item with the name: {} already exists".format(
                name.capitalize())}, 400  # Bad request client error
        ...
        try:
            item.save_to_db()  # Inserts itself
        except:
